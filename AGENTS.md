...
- **Golden test regen:** `tests/golden.rs` pins a recorded stream's rendered cell grid against `tests/fixtures/stream.golden`. When the rendered output changes intentionally, regenerate with `PLANK_REGEN_GOLDEN=1 cargo test -p turbo-debug-console`.

## Architecture

A Turbo Vision TUI that listens on a fixed loopback control port (7878), performs a line handshake, allocates a per-session data port, and renders the streamed bytes in a window. The crate is both a library (`turbo_debug_console`, `src/lib.rs`) and a binary (`src/main.rs`); the lib exists so integration tests drive the pipeline and protocol without a terminal.
